...
values('The Hamlet', '2.58', '1');

insert into track(name, duration, album_belonging)
values('A Brief Respite', '2.18', '1'), ('Return to the Warrens', '1.22', '1'),
('The Final Combat', '5.21', '1'), ('Town in Chaos', '6.08', '1'),
('A Mocking Thirst', '0.56', '1'), ('All Things Must Come', '3.48', '1');

insert into track(name, duration, album_belonging)
values('Dirtmouth', '1.55', '2'), ('Greenpath', '3.37', '2'),
('Mantis Lords', '1.45', '2'), ('City of Tears', '2.58', '2'),
('Soul Sanctum', '4.30', '2'), ('Queen''s Gardens', '1.46', '2'),
('Radiance', '2.17', '2');

insert into track(name, duration, album_belonging)
values('Entregarás tu rostro a la señora', '3.25', '3'),
('Cantes de confesión', '5.35', '3'), ('Arpegios en ocre', '3.35', '3'),
('Para un mártir del compás', '5.30', '3'),
('Al compás de tus contornos', '4.48', '3'),
('Y yo fuego te daré', '3.42', '3'),
('Su beso de plata', '3.13', '3');

insert into track(name, duration, album_belonging)
values('Cold Iron', '3.03', '4'), ('Ease', '2.38', '4'), ('Hokabi', '2.21', '4'),
('Innocence', '2.59', '4'), ('Letting My Heart Speak', '2.37', '4');
...
SELECT * FROM artist_genre
  inner JOIN artist 
    ON artist_genre.artist_id = artist.artist_id
  inner JOIN genre
    ON artist_genre.genre_id = genre.genre_id where genre.genre_id = 1

SELECT * FROM artist_genre
  inner JOIN artist 
    ON artist_genre.artist_id = artist.artist_id
  inner JOIN genre
    ON artist_genre.genre_id = genre.genre_id where genre.genre_id = 2

SELECT * FROM artist_genre
  inner JOIN artist 
    ON artist_genre.artist_id = artist.artist_id
  inner JOIN genre
    ON artist_genre.genre_id = genre.genre_id where genre.genre_id = 3

SELECT * FROM artist_genre
  inner JOIN artist 
    ON artist_genre.artist_id = artist.artist_id
  inner JOIN genre
    ON artist_genre.genre_id = genre.genre_id where genre.genre_id = 4

SELECT * FROM artist_genre
  inner JOIN artist 
    ON artist_genre.artist_id = artist.artist_id
  inner JOIN genre
    ON artist_genre.genre_id = genre.genre_id where genre.genre_id = 5
    
    SELECT * FROM artist_genre
  inner JOIN artist 
    ON artist_genre.artist_id = artist.artist_id
  inner JOIN genre
    ON artist_genre.genre_id = genre.genre_id where genre.genre_id = 6
    
SELECT * FROM artist_genre
  inner JOIN artist 
    ON artist_genre.artist_id = artist.artist_id
  inner JOIN genre
    ON artist_genre.genre_id = genre.genre_id where genre.genre_id = 7
    
SELECT * FROM artist_genre
  inner JOIN artist 
    ON artist_genre.artist_id = artist.artist_id
  inner JOIN genre
    ON artist_genre.genre_id = genre.genre_id where genre.genre_id = 8
    
SELECT track.name, track.album_belonging FROM track
...
SELECT avg(track.duration) FROM track
  JOIN album ON track.album_belonging = album.album_id where album.album_id = 1
    
SELECT avg(track.duration) FROM track
  JOIN album ON track.album_belonging = album.album_id where album.album_id = 2
  
SELECT avg(track.duration) FROM track
  JOIN album ON track.album_belonging = album.album_id where album.album_id = 3
  
SELECT avg(track.duration) FROM track
  JOIN album ON track.album_belonging = album.album_id where album.album_id = 4

SELECT artist.name FROM album_artist
  inner JOIN album 
    ON album_artist.album_id = album.album_id
  inner JOIN artist
    ON album_artist.artist_id = artist.artist_id where album.release_date not between '2020-01-01' and '2020-12-31'
    
SELECT compilation.name FROM compilation_track, track, album_artist
  inner JOIN compilation
  ...
  inner JOIN album
    ON album_artist.album_id = album.album_id
  inner JOIN artist
    ON album_artist.artist_id = artist.artist_id where artist.artist_id = 1
